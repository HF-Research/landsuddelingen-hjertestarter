...
Instead of trying to predict which groups receive assistance, we instead recommend using 2020 focus on reducing the number of no-shows among collectors. This may potentially be achieved by implementing an intervention that increases the percentage of collectors who are attached to a hjertestarter group. Additionally, it would be possible to test whether this intervention was effective by only applying it to a subset of the collectors
...
For the 2020 LU, we recommend aiming to reduce the number of no-shows. This could potentially be done with an intervention that increases the probability that a collector will join a hjertestarter group. the intervention could be applied on a rolling-basis to each newly recruited collector, or it could be applied at-once to all collectors that are registered (but not part of a hjertestarter group) on a specific date.The nature of this implementation is, however, beyond the scope of this analysis.

Additionally, it may be helpful to collect additional variables from the collectors that may help explain no-shows ahead of time. These could include:
...
Additionally, it would be relatively easy to assess the effectiveness of any intervention. If the intervention is applied to a randomly-selected sub-set of the collectors, then one can test if the intervention was associated with the no-show rate. This test can be done through a logistic regression fit to the dataset of collectors. The outcome variable would be the show/no-show status, and the explainitory variables would be the intervention group each collector belonged to.


testing to see if the planned intervention is effective at improving the likelihood that a group will achieve 15 routes (or if the intervention increases the number of routes a group achieves on average). This can be done in the following steps in 2020:
