...
# 
gender.path <- list.files("data/", full.names = TRUE, pattern = "gender")
out.names <- readRDS(gender.path)

api_size <- 999
first_name <- list()
tmp <- d$indsamlerny[hjertestarterid!=0, .(fornavn)]
...
# saveRDS(out.names, file = "data/gender_dataset.rds")
 

d$indsamlerny <- merge(d$indsamlerny, out.names, by.x = "name_gender", by.y = "name", all.x = TRUE)
# no_confi <- rbind(
#   out.names[count <5],
